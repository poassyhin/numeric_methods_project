...
def find_roots_in_range(M, f, a, b, EPS=1e-6):
    ...
    if root != None:
        return (
            [root]
            + find_roots_in_range(M, f, a, root - 2 * EPS, EPS)
            + find_roots_in_range(M, f, root + 2 * EPS, b, EPS)
        )
    else:
        return []
...
def solve(f, a, b, e):
    ...
    sol.update(
        {
                "dichotomy": filter_roots(
                        find_roots_in_range(dichotomy, f, a, b, e), f, e
                )
        }
    )
    sol.update(
        {
            "newton_method": filter_roots(
                find_roots_in_range(newton_method, f, a, b, e), f, e
            )
        }
    )
    sol.update(
        {
            "tangent_method": filter_roots(
                find_roots_in_range(tangent_method, f, a, b, e), f, e
            )
        }
    )
